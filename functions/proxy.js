...
// This Worker function acts as the server-side proxy for Cloudflare Pages Functions.
// It includes HTMLRewriter logic to fix relative links for images, CSS, and search forms.
...
export async function onRequest(context) {
    ...
    // Base URL for the proxy worker (used for rewriting links) - Pages uses the context.request.url.origin
    const proxyWorkerBaseUrl = url.origin + url.pathname.split('?')[0];

    try {
        // Perform the network request from the server
